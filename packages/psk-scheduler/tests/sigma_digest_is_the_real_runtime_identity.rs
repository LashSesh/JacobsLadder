...
/// (Vertrag 22.4).
///
/// PSK-RA v1.0.20 (Fehlerkorrektur 24, Regel 7.39) hat das an der Quelle
...
/// Regel 7.39, beide Haelften an EINEM Segment gemessen - der Test, der
...
fn segment_digest_excludes_the_wall_clock_and_segment_record_digest_includes_it() {
    ...
    for (x, y) in sa.iter().zip(sb.iter()) {
        assert_eq!(
            ...
            "segment_digest ist identitaetsbildend und DARF tau_e nicht tragen (Regel 7.39)"
        );
        assert_ne!(
            x.segment_record_digest, y.segment_record_digest,
            "segment_record_digest sichert die vollstaendigen Bytes und MUSS tau_e tragen (Regel 7.39)"
        );
    }
