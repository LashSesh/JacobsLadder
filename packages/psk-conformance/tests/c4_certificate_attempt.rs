...
//! dessen realem `boot_report`. OBL-010s beide fuer Regel 7.53 (plattformgebundeneverpflichtungsaufloesungimzertifikat)
...
/// Gegenstand Regel 7.53 (plattformgebundeneverpflichtungsaufloesungimzertifikat) ist: sie muessen den C4-Zweig erreichen, um
...
/// Gegenprobe zu Regel 7.53 (plattformgebundeneverpflichtungsaufloesungimzertifikat) - ohne sie bliebe unbewiesen, dass die Regel
/// im obigen Erfolgsfall ueberhaupt gefeuert hat statt bloss nicht
/// gestoert zu haben.
///
/// Dieselben Eingaben, EIN Unterschied: `scope` verschweigt die
/// Plattformbindung. Regel 7.53 (plattformgebundeneverpflichtungsaufloesungimzertifikat) verlangt dann Verweigerung - "sonst
...
fn the_same_c4_claim_is_refused_when_scope_hides_the_platform_binding() {
    ...
    assert_eq!(
        ...
        "Regel 7.53 (plattformgebundeneverpflichtungsaufloesungimzertifikat): ohne die Plattformbindung im scope MUSS die Ausstellung scheitern"
    );
}

/// Und die zweite Haelfte von Regel 7.53 (plattformgebundeneverpflichtungsaufloesungimzertifikat): auf einer Plattform OHNE
...
fn the_same_c4_claim_is_refused_on_a_foreign_platform() {
    ...
    assert_eq!(
        outcome.err(),
        Some(psk_types::PskError::ReleaseGateBlocked),
        "Regel 7.53 (plattformgebundeneverpflichtungsaufloesungimzertifikat): ohne deklarierte Aufloesung auf dieser Plattform ist C4 nicht erreicht"
    );
}
