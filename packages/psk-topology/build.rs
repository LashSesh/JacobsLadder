...
fn main() {
    ...
    let stubs =
        psk_codegen::generate_port_stubs_for_package(&module_map, &port_registry, "psk-topology");
    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    fs::write(out_dir.join("port_stubs.rs"), stubs).unwrap();

    let topology = psk_codegen::load_m13_topology(&root);
    fs::write(
        out_dir.join("m13_topology.rs"),
        psk_codegen::generate_m13_topology(&topology),
    )
    .unwrap();
}
